...
const UserLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("userToken", res.data.token); // assuming backend returns a JWT
      setMessage("");
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center">
      <div className="bg-gray-800 p-10 rounded-2xl shadow-2xl w-full max-w-md">
        <h2 className="text-3xl font-bold text-center text-white mb-6">User Login</h2>
        ...
        <p className="mt-4 text-center text-gray-300 text-sm">
          Don't have an account? <a href="/signup/user" className="text-indigo-500 font-medium hover:underline">Signup</a>
        </p>
      </div>
    </div>
  );
